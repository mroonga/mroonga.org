...
Mroonga 5.10以前では、インデックステーブルの区切り文字に`-`（ハイフン）が使われていました。`-`が含まれたテーブル名はスクリプト構文（`mroonga_command`）で使うことができない（シンタックスエラーになる）ため、スクリプト構文でインデックステーブルが使えませんでした（[自分でテーブルを作成するという回避策](https://osdn.jp/projects/groonga/lists/archive/dev/2015-December/003718.html)はありました）。Mroonga 5.11からはインデックステーブルの区切り文字がが`-`から`#`に変更されたため、スクリプト構文でインデックステーブルを使えるようになりました。
